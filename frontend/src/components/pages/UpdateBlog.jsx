  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        setParaOneDescription(data.blog.paraOneDescription); data.blog.paraOneImage && setParaOneImage(data.blog.paraOneImage.url);
        setParaTwoTitle(data.blog.paraTwoTitle);
        setParaTwoDescription(data.blog.paraTwoDescription); data.blog.paraTwoImage && setParaTwoImage(data.blog.paraTwoImage.url);
        setParaThreeTitle(data.blog.paraThreeTitle);
        setParaThreeDescription(data.blog.paraThreeDescription); data.blog.paraThreeImage &&
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://news-blog-backend-sooty.vercel.app/api/v1/blog/update/${id}`,
        updatedBlog,
        { withCredentials: true }
      );
            ...
            <img
              src={mainImagePreview
                ...
                : mainImage // Otherwise, use paraOneImagePreview
                  ? `${mainImage}`: "/imgPL.webp" // If neither paraOneImage nor paraOneImagePreview exists, use an empty string
              }
              alt="subParaOneImg"
            />
            <input
